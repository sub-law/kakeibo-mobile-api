...
class AssetBalanceApiTest extends TestCase
{
    ...
    public function test_authenticated_user_can_list_only_their_accounts(): void
    {
    ...
    public function test_account_cannot_be_created_without_an_owner(): void
    {
    ...
    public function test_user_cannot_register_balance_for_another_users_account(): void
    {
        ...
        $response = $this->actingAs($user, 'sanctum')
            ->postJson('/api/asset-balances/bulk', [
                'date' => '2026-07-01',
                'balances' => [
                    ['account_id' => $otherAccount->id, 'amount' => 100000],
                ],
            ])
            ->assertUnprocessable()
            ->assertJsonValidationErrors(['balances.0.account_id']);

        $errors = $response->json('errors');
        $this->assertSame(
            '指定された口座が存在しません。',
            $errors['balances.0.account_id'][0]
        );
    ...
    {
        $user = User::factory()->create();
        $accounts = Account::factory()->for($user)->count(3)->create();
    ...
    public function test_user_can_list_only_their_balances_for_the_requested_month_in_account_order(): void
    {
        ...
        AssetBalance::factory()->for($otherUser)->for($otherAccount)->create([
            'date' => '2026-07-01',
        ]);
